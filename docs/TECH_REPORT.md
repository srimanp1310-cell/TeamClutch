...
Twelve dispatch keys, two distinct winners. Generated by
`analysis.load.dispatch_choices` and reproduced in `results/summary.md`:
...
| `8,512,512,8,float32,True,False` | optimized | 1.003x | — | — |
| `8,512,512,8,float32,True,True` | optimized | 1.002x | — | — |
| `8,512,512,8,bfloat16,False,False` | optimized | 1.000x | — | — |
| `8,128,512,8,float32,False,False` | optimized | 0.984x | sdpa | 0.074 |

One row deserves a caveat rather than a victory lap. The `float16` key shows
`sdpa` winning by 1.009 — but that margin exists only because the measurement
behind it is a *single-layer* run, and `DispatchKey` carries no layer count
(§5). At the benchmark's six-layer default, fp16 is inadmissible and the router
correctly refuses it via the depth rule in `src/optimized.py`. The table records
what was measured; the shipped path does not follow it there.

The margin column is the honest part. A key won by 0.002x is a tie: the choice
there is inside run-to-run noise and should not be read as a result. **Seven of
the twelve keys are won by 0.05x or more.** Two are ties under that rule — the
`d_model=1024` key at 0.041 and the base `8,512,512` fp32 key at 0.042, where
either strategy would do. The remaining three have no runner-up at all: only one
strategy produced a passing measurement for that key, so there is nothing to
compare and the choice is made by default rather than by evidence.
...
- Across the fifteen configurations the shipped router actually serves, the
  worst element is **0.000798 absolute** — **79.8% of the `atol` budget**, so a
  20% margin against the stricter rule and a 2.5x margin against the problem
  statement's own. Every one of the fifteen passes.
- **Absolute error only, deliberately.** `max_rel_err` is recorded in
  `results.csv` but is not reportable as a result: it divides by
  `ref.abs().clamp_min(1e-12)`, so wherever a reference output sits near zero
  the ratio explodes — the worst value in the log is **1.7e+07**, which is a
  property of the denominator and not of any implementation. The pass rule is
  OR-combined, so those elements are judged on their absolute error and pass
  comfortably; it is only the *summary statistic* that is meaningless. Quoting
  it would suggest a catastrophic failure that did not occur.
...
Discarded rows are kept rather than deleted, and counted: **zero rows out of 77
carry `DISCARD:thermal`.** No run in the entire log tripped the rule.

That zero is worth reading carefully, because it can be taken two ways. It is
not evidence that the defences were unnecessary — a discard rule that never
fires and a discard rule that does not work look identical from the outside, and
the only reason we can tell them apart is that `mean_sm_clock_mhz` and
`max_temp_c` are recorded on every row and can be inspected directly (§8.1).
What the clock columns show is that the card ran *hotter and slower* on some
configurations than others without ever falling 15% below its own opening clock
within a single timed window, which is the specific thing the rule tests for.

The instability that did cost us a result was not thermal at all: the reference
implementation got 7.4% faster between two sessions while our own execution time
moved by 0.2%, which moved a speedup ratio through its denominator (§8.1). Four
defences against throttling, none of which fire, and the one measurement problem
we actually hit is one none of them addresses. That is a more useful thing to
report than a clean bill of health.
...
Summary: Claude, through Claude Code (Opus 5) and Claude chat. No other AI tool
was used. It wrote most of the first draft of every file in `bench/`, `src/`,
`analysis/` and `docs/`, and every kernel decision, tolerance target and roofline
number was checked against a measurement before it survived.

The corrections are the useful part, and they fall into a pattern: the model was
reliable at producing code that ran and unreliable at noticing when a *test* or
an *aggregation* was quietly meaningless.

- **A guard that could not fail.** The oracle's self-test scaled the output by
  1.001 to prove the comparison can detect an error. But the pass rule is an OR,
  and 0.1% sits comfortably inside `rtol = 0.01`, so the "failure" case passed.
  Reworked to 1.02, which clears both legs wherever `|ref| > 0.05`, turning a
  smoke test into a sensitivity bound.
- **An aggregation that silently swapped the denominator.** A
  `--compile-baseline` run shares a configuration key with its eager
  counterpart and happens later, so `latest_per_config` treated it as a newer
  run of the same experiment and dropped the eager row. Every speedup line was
  quietly reporting against a compiled baseline instead of the eager one the
  organizers' script uses.
- **A cache that discarded a safety check.** To make an LRU key hashable, the
  available strategies were passed as a tuple of *names* and the classes looked
  up again from the global registry — which throws away any explicitly-passed
  mapping, so a strategy declaring `MIN_CAPABILITY = (9, 0)` was selected on an
  sm_89 card.
- **A parser that reported a catastrophe instead of a gap.** `gpu_busy_fraction`
  returned 0.0% on the real traces, because WSL2's CUPTI populates no
  device-side kernel records at all. Zero percent busy reads as the worst
  possible result rather than a missing measurement; it now returns `NaN` and
  every consumer prints "unmeasurable".
- **An assumption that a claim needed data it did not.** §1's central argument
  was left as a placeholder pending measurements, when it is derivable
  analytically: the reference's arithmetic intensity *falls* from 64.6 to 32.3
  FLOP/byte as S goes 512 → 4096 while a fused path's *rises* from 113.8 to
  237.4, crossing this card's 62.9 FLOP/byte ridge. The whole optimization
  ladder was predictable before a single kernel ran.

Two further corrections in the same family are recorded in §4 Rung 3 and §4.1:
a launch count that undercounted by 42% by matching one API name, and an
aggregation that filtered to passing rows *before* taking the newest, so a
corrective FAIL could never supersede the PASS it was written to correct.
